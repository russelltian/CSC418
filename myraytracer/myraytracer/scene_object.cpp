...
bool UnitSphere::intersect(Ray3D& ray, const Matrix4x4& worldToModel,
    ...
    normal.normalize();
    double u=atan2(normal[2],normal[0])/(2*PI);
...
SceneNode::SceneNode(UnitCube* inobj, Material* mat){
    this->firstTouch=true;
    this->obj=inobj;
    this->mat=mat;
    this->ismesh=false;
...
bool UnitCube::intersect(Ray3D &ray, const Matrix4x4 &worldToModel, const Matrix4x4 &modelToWorld){
    ...
    // direction.normalize();
    double threshold = 0.00000001;
    int side = -1; // first side being hit, 0,1,2 for x,y,z
    ...
    side = 0;//x plane is the first hit
    // cout << "hit x first ";
    ...
    if(tmin == min(tz1,tz2)){
        side = 2; //first hit z plane
        //  cout << "hit z first\n";
    }
    if(tmax < threshold)return false;
    ...
    if(tmin > threshold){
        ...
        double absY = fabs(y);
        double absZ = fabs(z);
        Point3D p(origin + tmin*direction);
        Vector3D normal;
        if(side == 0){
